...
import sys
from plain.SpriteSheet import SpriteSheet
...
class Plain:
  ...
  def _initDisplay(self):
    ...
    self.tileImages = []
    ss = SpriteSheet(self.level['graphicsMap']+'/'+self.level['tileImages'])
    for tile in range(len(self.level['tileTypes'])):
      y = tile * self.tileSize
      self.tileImages.append(ss.load_strip((0,y,64,64), 1, (0,0,0)))

    ss = SpriteSheet(self.level['graphicsMap'] +'/'+ self.level['player']['img'])
    ...
    ss = SpriteSheet(self.level['graphicsMap'] +'/'+ self.level['goal']['img'])
    self.goalImages = ss.load_strip((0,0,64,64), 1, (0,0,0))
    self.goalReached = False

    ss = SpriteSheet(self.level['graphicsMap'] +'/'+ self.level['collision']['img'])
